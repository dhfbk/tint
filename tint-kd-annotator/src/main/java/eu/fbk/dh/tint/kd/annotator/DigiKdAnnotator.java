...
public class DigiKdAnnotator implements Annotator{
    ...
    public DigiKdAnnotator(String annotatorName, Properties prop) {
        ...
        configuration.skip_keyword_with_proper_noun = Boolean.parseBoolean(prop.getProperty(annotatorName+".skip_keyword_with_proper_noun","false"));
        configuration.skip_proper_noun = Boolean.parseBoolean(prop.getProperty(annotatorName+".skip_proper_noun","false"));
    ...
    public void annotate(Annotation annotation) {
        ...
        if (annotation.has(CoreAnnotations.SentencesAnnotation.class)) {
            for (CoreMap sentence : annotation.get(CoreAnnotations.SentencesAnnotation.class)) {
                ...
                for (CoreLabel c : tokens) {
                    ...
                    doc.append(c.word()+"\t"+c.get(CoreAnnotations.LemmaAnnotation.class)+"\t"+c.get(CoreAnnotations.PartOfSpeechAnnotation.class)+"\n");
                }
                doc.append("\n");
            }
        }
        ...
        try {
            for (KD_keyconcept k : kd.extractExpressions(this.lang, this.configuration, null, doc)) {
                listOfKeys.add(new DigiKdResult(k.getString(), k.frequency, k.score, k.getLemmaArray(), k.getTokenArray(), k.getPosList()));
            }
    ...
    public Set<Requirement> requires() {
        return Collections.unmodifiableSet(new ArraySet<Requirement>(TOKENIZE_REQUIREMENT,LEMMA_REQUIREMENT,SSPLIT_REQUIREMENT, POS_REQUIREMENT));
