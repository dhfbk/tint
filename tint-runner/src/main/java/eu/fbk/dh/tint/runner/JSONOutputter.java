...
//    @SuppressWarnings("RedundantCast")  // It's lying; we need the "redundant" casts (as of 2014-09-08)
    ...
    public void print(Annotation doc, OutputStream target, Options options) throws IOException {
        ...
        l0.object(l1 -> {
            ...
            if (doc.get(CoreAnnotations.SentencesAnnotation.class) != null) {
                l1.set("sentences", doc.get(CoreAnnotations.SentencesAnnotation.class).stream()
                        .map(sentence -> (Consumer<Writer>) (Writer l2) -> {
                            ...
                            // (dependency trees)
                            l2.set("basic-dependencies", buildDependencyTree(
                                    sentence.get(SemanticGraphCoreAnnotations.BasicDependenciesAnnotation.class)));
                            l2.set("collapsed-dependencies", buildDependencyTree(
                                    sentence.get(SemanticGraphCoreAnnotations.CollapsedDependenciesAnnotation.class)));
                            l2.set("collapsed-ccprocessed-dependencies", buildDependencyTree(sentence.get(
                            ...
                            if (openIETriples != null) {
                                l2.set("openie", openIETriples.stream()
                                        .map(triple -> (Consumer<Writer>) (Writer tripleWriter) -> {
                            ...
                            if (sentence.get(CoreAnnotations.TokensAnnotation.class) != null) {
                                l2.set("tokens", sentence.get(CoreAnnotations.TokensAnnotation.class).stream()
                                        .map(token -> (Consumer<Writer>) (Writer l3) -> {
                                            // Add a single token
                ...
                if (corefChains != null) {
                    l1.set("corefs", (Consumer<Writer>) chainWriter -> {
                        for (CorefChain chain : corefChains.values()) {
                            ...
                            chainWriter.set(Integer.toString(chain.getChainID()),
                                    chain.getMentionsInTextualOrder().stream()
                                            .map(mention -> (Consumer<Writer>) (Writer mentionWriter) -> {
                                                mentionWriter.set("id", mention.mentionID);
                                                mentionWriter.set("text", Sentence
                                                ...
                                                mentionWriter.set("position",
                                                        Arrays.stream(mention.position.elems()).boxed()
    ...
    protected static class JSONWriter {
        ...
        public static String objectToJSON(Consumer<Writer> callback) {
            ...
            new JSONOutputter.JSONWriter(out, new Options()).object(callback);
            out.close();
            return os.toString();
        }
    }
